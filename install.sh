#!/usr/bin/env bash
# ascension — installer. Wires this skill pack into your agent harnesses.
...
#   (/plugin marketplace add mtthsnc/ascension ; /plugin install ascension@ascension).
...
say() { printf '  %s\n' "$*"; }
echo "ascension: installing from $REPO"
...
link_skills() {
  ...
  for s in "$SKILLS_SRC"/*/; do
    ...
    if [ -L "$link" ]; then
      ...
      mv "$link" "$link.pre-ascension.bak"; say "backed up existing $name -> $name.pre-ascension.bak"
    fi
    ln -sfn "${s%/}" "$link"
  done
}
